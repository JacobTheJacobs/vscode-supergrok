...
// ACP that src/acp/client.ts actually exercises:
...
// optionally with `--reasoning-effort <value>` (agent-level flags, before the
...
// Accept `agent [--leader] [--reasoning-effort <valid>] stdio` in any order of
// the optional flags before the trailing `stdio` subcommand.
function argvOk(a) {
  if (a[0] !== "agent" || a[a.length - 1] !== "stdio") return false;
  const mid = a.slice(1, -1);
  for (let i = 0; i < mid.length; ) {
    if (mid[i] === "--leader") { i += 1; continue; }
    if (mid[i] === "--reasoning-effort" && VALID_EFFORT.has(mid[i + 1])) { i += 2; continue; }
    return false;
  }
  return true;
}
