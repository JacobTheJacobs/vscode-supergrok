...
describe("slash command menu", () => {
  ...
  it("opens from the toolbar slash button and shows every Grok command", () => {
    ...
    expect(Array.from(pop.querySelectorAll(".slash-section-title")).map((el) => el.textContent))
      .toEqual(["Settings", "Model", "Session", "Memory", "Permissions"]);
  });
...
describe("slash settings menu — Config & debug", () => {
  ...
  it("shows settings actions in slash without a settings button or version-details entry", () => {
    ...
    expect(labels).toContain("Ctrl/Cmd+Enter to send");
    expect(labels).toContain("Attach active file by default");
    ...
    click(h.window, slashItem(h.doc, "Ctrl/Cmd+Enter to send"));
    expect(h.posted).toContainEqual({ type: "updateConfig", key: "useCtrlEnterToSend", value: true });
  });
