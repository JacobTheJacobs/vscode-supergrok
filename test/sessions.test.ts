...
describe("isInternalPrimerSummary", () => {
  it("recognizes hidden primer summaries generated by Grok", () => {
    expect(isInternalPrimerSummary("Supergrok Primer v5 Plan Mode Instructions")).toBe(true);
    expect(isInternalPrimerSummary("[vscode-supergrok primer v5]\n\n## HIDDEN PRIMER")).toBe(true);
  });
  it("does not hide normal user summaries that mention plans", () => {
    expect(isInternalPrimerSummary("Plan mode instructions for the auth refactor")).toBe(false);
  });
});
...
describe("listSessions", () => {
  ...
  it("falls back to untitled when Grok summarizes the hidden primer as the session title", () => {
    const fs = buildFs({
      [dir]: { isDir: true },
      [dirFor("a")]: { isDir: true },
      [path.join(dirFor("a"), "summary.json")]: {
        ...
        content: JSON.stringify({
          info: { id: "a", cwd },
          session_summary: "Supergrok Primer v5 Plan Mode Instructions",
