...
See design doc for the full state machine diagram.`;
  ...
  private async maybeUpdateCliOnUpgrade(cliPath: string): Promise<void> {
    // Silent grok-CLI auto-update is intentionally DISABLED: it can push the CLI
    // to a build that breaks the stdio agent (e.g. 0.2.60). Update grok yourself
    // via `grok update` when you choose to. We still record the extension version
    // so re-enabling later behaves predictably.
  ...
  private async onMessage(msg: WebviewMsg): Promise<void> {
    switch (msg.type) {
      ...
      case "openSkillsFolder": {
        // Grok loads skills from ~/.claude/skills (claude harness compat is on by
        // default). Reveal/create it so the user can add or edit skill folders.
        const home = process.env.HOME || process.env.USERPROFILE || "";
      ...
      case "runMcpList": {
        // Run the CLI directly and stream the result into the output channel.
        // Launching grok as a terminal "shell" exits immediately (output flashes
        // and dies), and a bare `grok mcp list` fails when grok isn't on the
        // terminal's PATH — so neither survives. A child process is shell-agnostic.
        ...
        this.output.appendLine("\n$ grok mcp list");
        try {
          const { stdout, stderr } = await execFileAsync(mcpCli, ["mcp", "list"], {
            cwd: mcpCwd,
            timeout: 15_000,
            windowsHide: true,
          });
          this.output.appendLine(`${stdout ?? ""}${stderr ?? ""}`.trim() || "(no output)");
        } catch (e: any) {
          this.output.appendLine(`${e?.stdout ?? ""}${e?.stderr ?? ""}${e?.message ?? ""}`.trim() || "grok mcp list failed");
        }
        break;
      }
      case "showLogs":
      ...
      <div class="toolbar-left">
        ...
        <div class="context-donut" id="donut" aria-label="Context usage">
          <svg width="13" height="13" viewBox="0 0 13 13">
            <circle cx="6.5" cy="6.5" r="5" fill="none" stroke="color-mix(in srgb, var(--vscode-foreground,#fff) 24%, transparent)" stroke-width="1.6"/>
            <circle id="donut-arc" cx="6.5" cy="6.5" r="5" fill="none" stroke="var(--sg-accent,#fff)" stroke-width="1.8" stroke-linecap="round" stroke-dasharray="0 999" transform="rotate(-90 6.5 6.5)"/>
          </svg>
