(function () {
  ...
  const DEFAULT_SLASH_COMMANDS = [
    ...
    { name: "always-approve", description: "Toggle always-approve mode", group: "Permissions" },
  ];
  const SLASH_GROUP_ORDER = ["Settings", "Model", "Session", "Context", "Memory", "Permissions", "Tools", "Commands"];
  ...
  function renderConfigDebugPanel(backAction = reopenSlashFromSettings) {
    ...
    addSettingsItem("<span>Show extension logs</span>", () => {
  ...
  function updateDonut(used) {
    ...
    donutArc.setAttribute("stroke", "var(--vscode-descriptionForeground)");
  ...
  function localSlashCommands() {
    ...
    return [
      ...
      },
      {
        ...
        description: "When enabled, Enter inserts a newline and Ctrl/Cmd+Enter sends",
        group: "Settings",
        action: "toggle",
        key: "useCtrlEnter",
        configKey: "useCtrlEnterToSend",
        checked: state.useCtrlEnter,
      },
      {
        name: "include-active-file",
        label: "Attach active file by default",
        description: "Automatically add the current editor file as context",
        group: "Settings",
        action: "toggle",
        key: "includeActiveFileByDefault",
        configKey: "includeActiveFileByDefault",
        checked: state.includeActiveFileByDefault,
      },
      {
        name: "log-out",
        label: "Log out",
  ...
  function slashCommandGroup(cmd) {
    ...
    if (/(approve|permission|allow|deny|yolo)/.test(name)) return "Permissions";
  ...
  function normalizeSlashCommand(cmd) {
    ...
    return {
      ...
      configKey: typeof cmd === "string" ? "" : (cmd && cmd.configKey),
  ...
  function allSlashCommands() {
    ...
    const source = [...localSlashCommands(), ...cliCommands];
    const seen = new Set();
  ...
  function runSlashAction(cmd) {
    switch (cmd.action) {
      ...
      case "toggle": {
        ...
        if (cmd.configKey) vscode.postMessage({ type: "updateConfig", key: cmd.configKey, value: next });
        const m = slashQueryAtCursor();
        showSlashCommands(m ? m[1] : "");
        return;
      }
  ...
  window.addEventListener("message", (e) => {
    ...
    switch (msg.type) {
      case "initialState":
        ...
        state.includeActiveFileByDefault = msg.includeActiveFileByDefault !== false;
